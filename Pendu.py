...
#Creation de la base de donnees('affichage du joueur gagnant')
connection = sqlite3.connect("test.db")
print(connection.total_changes)
cursor = connection.cursor()
cursor.execute("DROP TABLE IF EXISTS players")
cursor.execute("CREATE TABLE IF NOT EXISTS players (name TEXT, session TEXT)")
connection.commit()
...
# cadre pour les inputs de leur 'Nom_joueur'
...
# Ajout fenetre  'Game' dans la  frame 2
label = tk.Label(frame2, text="Game", bg="GREEN", width=20)
label.place(x=360, y=10, height=30)


# Intégration du jeu dans la frame2
def progres(mot_en_progres, lettre, secret):
...
def score(lettre):
    count = 0
    global x, end, img, mot_en_progres, joueur_score_scores
    if lettre not in secret:
        cnv.delete(images[x])
        x -= 1
        cnv.create_image((small_width_img / 2, small_height_img / 2), image=images[x])
        if x == 0:
        ...
        if joueur_score_scores[current_joueur] == 2:
            cnv.create_text(small_width_img / 2, small_height_img / 2, text=f"{joueur[current_joueur]} Wins!",
                            font=('Deja Vu Sans Mono', 30, 'bold'), fill="green")
            display_winner(joueur[current_joueur], count)
        else:
            next_player()
...
def next_player():
    ...
    current_joueur = (current_joueur + 1) % 4
    end = False
    player_display()
    init()
...
def choisir_lettre(event):
    ...
    progres(mot_en_progres, lettre, secret)
    label["text"] = "".join(mot_en_progres)
    score(lettre)


def init():
    global end, mot_en_progres, secret, x, img
...
def countdown():
    ...
    if time_left > 0:
        ...
        time_left -= 1
        frame2.after(2000, countdown)
    else:
        next_player()


def display_winner(winner_name, count):
    winner_button = tk.Button(frame2, text=f"{winner_name} a gagné", font=("Helvetica", 15), bg="green", fg="white",
                              command=lambda: show_frame(frame1))
    winner_button.place(x=400, y=385)

    # sauvegarder ('winner_name') a la db
    cursor.execute('INSERT into players VALUES (?,?)',(winner_name,'seance:' + str(count + 1)))
    connection.commit()
...
for j in range(limite + 1):
    try:
        img = tk.PhotoImage(file=f"{j}.png").subsample(4, 4)
        images.append(img)
...
small_width_img = win.width() if win else 200
small_height_img = win.height() if win else 150

cnv = tk.Canvas(frame2, width=small_width_img, height=small_height_img, highlightthickness=4, bg="#E6E6E6")
cnv.place(x=20, y=120)  # Image à gauche de l'interfac
...
label = tk.Label(frame2, font=("Helvetica", 20), bg="#E6E6E6")
label.place(x=520, y=120)  # Position du mot en cours
